(() => {
  ...
  function diagnosis(d){
    ...
    const candidates=offline&&weak?[['无线信号衰减 / 链路质量下降',72,'同区域 / 同网关异常更值得优先关注'],['网关侧连接波动',19,'建议关联同网关设备状态'],['设备自身故障',9,'单设备异常时再提高该假设权重']]:offline?[['网关侧连接中断',58,'该设备当前无上报，先确认同网关设备是否同步掉线'],['无线覆盖劣化 / 天线故障',26,'离线前的 RSSI 与丢包趋势是主要判据'],['设备断电或自身故障',16,'排除前两项后再提高该假设权重']]:hot?[['设备负载或执行机构异常',61,'温度与设备类型 / 运行状态需要联合判断'],['环境温度影响',23,'对比同区域设备温度曲线'],['传感器漂移',16,'检查校准记录与历史基线']]:[['通信质量变化',45,'检查 RSSI 与丢包趋势'],['网关连接状态',31,'关联同网关设备'],['设备自身状态',24,'检查电量 / 固件 / 重启记录']];
    const actions=offline&&weak?['确认所属网关在线与丢包情况','对比同网关设备异常数量','现场检查无线覆盖 / 天线','必要时执行重新连接']:offline?['确认所属网关在线与丢包情况','对比同网关其他设备的异常数量','现场确认设备供电与天线连接','必要时执行重新连接']:hot?['确认当前负载与运行工况','对比同区域同类型温度','检查散热与执行机构','建立预防性维护工单']:['检查近期遥测趋势','关联相邻资产与网关','确认是否需要现场巡检'];
