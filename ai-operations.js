(() => {
  ...
  function diagnosis(d){
    ...
    let primary='设备状态异常';
    if(offline&&weak) primary='通信链路 / 弱信号';
    else if(hot&&weak) primary='负载异常 + 通信质量下降';
    else if(offline) primary='通信链路中断';
    else if(hot) primary='设备温度异常';
    const candidates=offline&&weak?[
      ['无线信号衰减 / 链路质量下降',72,'同区域 / 同网关异常更值得优先关注'],
      ['网关侧连接波动',19,'建议关联同网关设备状态'],
      ['设备自身故障',9,'单设备异常时再提高该假设权重']
    ]:hot?[['设备负载或执行机构异常',61,'温度与设备类型 / 运行状态需要联合判断'],['环境温度影响',23,'对比同区域设备温度曲线'],['传感器漂移',16,'检查校准记录与历史基线']]:[
      ['通信质量变化',45,'检查 RSSI 与丢包趋势'],['网关连接状态',31,'关联同网关设备'],['设备自身状态',24,'检查电量 / 固件 / 重启记录']
    ];
  ...
  function ensurePanel(){
    ...
    p.innerHTML='<div class="ai-diagnosis-card"><button class="ai-close" id="ai-close">×</button><div class="ai-diag-top"><div><div class="eyebrow">AI DIAGNOSIS / DECISION SUPPORT</div><h2 id="ai-diag-title">设备诊断</h2><p id="ai-diag-sub">--</p></div><span class="ai-confidence" id="ai-confidence">--</span></div><div class="ai-diag-grid"><div><div class="ai-section-label">ROOT CAUSE HYPOTHESES</div><div id="ai-causes"></div></div><div><div class="ai-section-label">RECOMMENDED ACTIONS</div><div id="ai-actions"></div></div></div><div class="ai-evidence" id="ai-evidence"></div><div class="ai-action-bar"><button class="action" id="ai-topology">查看拓扑</button><button class="action" id="ai-alerts">查看事件</button><button class="action" id="ai-workorder">创建工单</button><button class="action primary" id="ai-confirm">确认根因 / 进入处置</button></div><div id="ai-toast" class="ai-toast"></div></div>';
  ...
  function open(id){
    ...
    $('#ai-evidence').innerHTML=`<span>Evidence</span><b>RSSI ${d.rssi??'--'} dBm</b><b>Temp ${d.temperature==null?'--':d.temperature+'°C'}</b><b>Last seen ${d.minutesSinceSeen??0} min</b><b>Risk ${Math.max(0,100-score(d))}% health</b>`;
    ...
    $('#ai-confirm').onclick=()=>{localStorage.setItem('iot-ai-confirmed',JSON.stringify({deviceId:d.id,rootCause:r.primary,at:Date.now()}));$('#ai-toast').textContent=`已确认：${r.primary}。建议下一步进入工单 / 现场处置。`};
